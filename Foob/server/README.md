...
A tiny Cloudflare Worker that lets friends & family use Foob **without
...
3. **Create the storage namespace** (holds invite codes + usage):
   ...
   ```bash
   cd /Users/alexnovak/Personal-Apps/Foob/server
...
5. **Deploy:**
   ...
   **That URL goes into the app** — set it as `ClaudeEndpoint.defaultProxyURL`
   in `Foob/Services/ClaudeEndpoint.swift` before building for TestFlight
