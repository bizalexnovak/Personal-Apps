...
final class SpeechCaptureController: ObservableObject {
    ...
    func start() async {
        ...
        guard speechStatus == .authorized else {
            state = .denied("Foob needs speech recognition permission to transcribe your meal. Enable it in Settings.")
            return
        }
        guard await AVAudioApplication.requestRecordPermission() else {
            state = .denied("Foob needs microphone access to hear your meal. Enable it in Settings.")
            return
        }
        beginSession()
    }
